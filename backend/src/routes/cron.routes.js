...
// Threshold in seconds between consecutive solves to consider rapid/suspicious
...
async function saveLeetCodeSubmissions(userId, username, submissions) {
  ...
  const rows = submissions.map((submission) => {
    ...
    // Flag rapid solves if not previously reviewed by mentor
    if (isRapid && !alreadyReviewed) {
...
async function updateLeaderboardSuspensionStatus(userId, profileId) {
  try {
    ...
    if (updateError) {
      logSupabaseError("Updating leaderboard status", updateError, {
        userId,
        profileId,
        hasPendingReviews,
      });
...
router.get("/queue", async (req, res) => {
  try {
    ...
    if (submissionError) {
      ...
      return res.status(500).json({ error: "Failed to fetch mentor review queue" });
    }
    ...
    const { data: squadStudents, error: squadError } = await supabaseAdmin
      .from("squad_students")
      .select("student_user_id, squad_id")
      .in("student_user_id", studentIds);

    if (squadError) {
      logSupabaseError("Fetching student squads", squadError);
      return res.status(500).json({ error: "Failed to fetch student squads" });
    }
...
router.patch("/:studentUserId/approve", async (req, res) => {
  ...
  try {
    ...
    if (error) {
      logSupabaseError("Approving mentor review", error, { studentUserId });
      return res.status(500).json({ error: "Failed to approve submissions" });
    }
...
router.patch("/:studentUserId/reject", async (req, res) => {
  ...
  try {
    ...
    if (error) {
      logSupabaseError("Rejecting mentor review", error, { studentUserId });
      return res.status(500).json({ error: "Failed to reject submissions" });
    }
...
// ============================================================
// PUBLIC LEADERBOARD
// Returns all students (including flagged/under review sorting)
// ============================================================
...
router.get("/leetcode-leaderboard", async (req, res) => {
  try {
    ...
    // Attach profile and pending review details to each leaderboard record
    ...
    // Custom Sort: Push under-review or suspended users to the bottom of the list
    result.sort((a, b) => {
      const aFlagged = a.is_under_review || a.is_suspended;
      const bFlagged = b.is_under_review || b.is_suspended;

      // Clean accounts first, flagged/suspended accounts last
    ...
    // Re-assign explicit positions so top numbers (1, 2, 3...) reflect actual active ranks
    result.forEach((entry, index) => {
      entry.display_rank = index + 1;
    });

    console.log(
      `[PUBLIC LEADERBOARD] Total: ${leaderboardData.length} | Pending Submissions: ${
...
async function notifyMentorsAboutInactiveStudents() {
  ...
  try {
    ...
    for (const assignment of assignments) {
      ...
      if (!studentId || !mentorId) continue;
          ...
          .map(
            (student) => `
                <li>
                  ...
                  (${student.email})
                  — Last active: ${student.lastSolved} (${student.daysInactive} days inactive)
                </li>
              `,
          )
          .join("");
          ...
          .map(
            (student) => `
                <li>
                  <strong>${student.name}</strong>
                  (${student.email})
                  — <em>LeetCode profile URL not set</em>
                </li>
              `,
          )
          .join("");
      ...
      const response = await fetch("https://api.brevo.com/v3/smtp/email", {
        ...
        body: JSON.stringify({
          ...
          htmlContent: `
                ...
                <h4>🚨 High Priority: Inactive for 7+ Days (${data.sevenDaysPlus.length})</h4>
                ${sevenDaysHtml}
                <h4>⚠️ Warning: Inactive for 1–6 Days (${data.oneToSixDays.length})</h4>
                ${oneToSixDaysHtml}
                <h4>📝 Action Required: LeetCode Profile Not Set (${data.missingProfile.length})</h4>
                ${missingProfileHtml}
                <p>Please reach out to your squad members to keep them on track.</p>
                <p>Regards,<br>Kalvium Portfolio Management</p>
              `,
        }),
      });
...
async function syncSingleLeetCodeProfile(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const { count: pendingCount, error: pendingReviewError } =
        await supabaseAdmin
          .from("leetcode_submissions")
          .select("*", { count: "exact", head: true })
          .eq("user_id", userId)
      ...
      return {
        ...
        score,
        pendingReviewCount,
        isUnderReview: hasPendingReviews,
